...
def process_mutations_in_batches(
    ...
    for batch_num in range(total_batches):
        ...
        if valid_mask.any():
            ...
            # Generate peptides
            # Optimization: Use list comprehension with zip and itertuples(name=None)
            # instead of .apply(axis=1) for significant speedup when applying functions element-wise over DataFrames.
            # This avoids the overhead of creating pd.Series and also avoids the mangled keys bug of itertuples namedtuples.
            cols = parsed_df.columns
            parsed_df["peptide"] = [
                generate_peptides(dict(zip(cols, row)))
                for row in parsed_df.itertuples(index=False, name=None)
            ]
