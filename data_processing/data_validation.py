...
def validate_peptides(peptides: pd.Series) -> Tuple[bool, Dict]:
    ...
    valid_mask = peptides.notna()
    invalid_aa = valid_mask & peptides.str.contains(r'[^ACDEFGHIKLMNPQRSTVWY]', regex=True, na=False)
    invalid_aa_count = invalid_aa.sum()
    
    if invalid_aa_count > 0:
        errors['invalid_amino_acids'] = invalid_aa_count
    
    # Check length (should be 9-mer) using vectorized string operations
    invalid_length = valid_mask & (peptides.str.len() != 9)
