...
def validate_mutations(df: pd.DataFrame) -> Tuple[bool, Dict]:
    ...
    valid_variants = pd.Series([parse_protein_change(x) is not None for x in df['HGVSp_Short']], index=df.index)
...
def cross_validate_mutations(raw_df: pd.DataFrame, processed_df: pd.DataFrame) -> Dict[str, int]:
    ...
    valid_mutations = pd.Series([parse_protein_change(x) is not None for x in raw_df['HGVSp_Short']], index=raw_df.index)
