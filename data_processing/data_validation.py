...
def validate_peptides(peptides: pd.Series) -> Tuple[bool, Dict]:
    ...
    # Check for valid amino acids (Optimized: Vectorized string operations instead of .apply loop)
    invalid_aa = peptides.str.contains(f"[^{''.join(VALID_AA)}]", regex=True).fillna(False)
    invalid_aa_count = invalid_aa.sum()
    
    if invalid_aa_count > 0:
        errors['invalid_amino_acids'] = invalid_aa_count
    
    # Check length (should be 9-mer) (Optimized: Vectorized string length check)
