...
def validate_mutations(df: pd.DataFrame) -> Tuple[bool, Dict]:
    ...
    valid_variants = df["HGVSp_Short"].apply(
        lambda x: parse_protein_change(x) is not None
    )
...
def validate_peptides(peptides: pd.Series) -> Tuple[bool, Dict]:
    ...
    invalid_length = peptides.apply(lambda x: False if pd.isna(x) else len(x) != 9)
...
def cross_validate_mutations(
    raw_df: pd.DataFrame, processed_df: pd.DataFrame
) -> Dict[str, int]:
    ...
    valid_mutations = raw_df["HGVSp_Short"].apply(
        lambda x: parse_protein_change(x) is not None
    )
    ...
    if stats["valid_mutations"] > 0:
        stats["processing_efficiency"] = (
            stats["valid_peptides"] / stats["valid_mutations"]
        ) * 100
