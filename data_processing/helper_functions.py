...
def generate_peptides(row: Any, peptide_length: int = 9) -> Optional[str]:
    ...
    try:
        ...
        logging.warning("Row missing expected attributes")
        return None

    if not isinstance(wt_seq_tuple, tuple) or len(wt_seq_tuple) != 2:
        logging.warning(f"Invalid or missing wildtype_seq for {hugo}")
        return None

    seq, seq_length = wt_seq_tuple
    if not seq or seq_length is None:
        logging.warning(f"Missing sequence or length for {hugo}")
        return None

    if pos > seq_length or pos <= 0:
        logging.warning(
            f"Isoform mismatch or invalid position: {hugo} "
    ...
    try:
        ...
        if mut_type == "Sub":
            ...
            if not cds_seq or pd.isna(cds_seq):
                logging.warning(f"Missing CDS for frameshift in {hugo}")
                return None
    ...
    if len(mutant_peptide) < peptide_length:
        logging.warning(
            f"Peptide too short for {hugo}: {len(mutant_peptide)} < {peptide_length}. Skipping."
        )
        return None
    ...
    if not all(aa in VALID_AA for aa in mutant_peptide):
        logging.warning(f"Invalid peptide generated for {hugo}: {mutant_peptide}")
        return None
