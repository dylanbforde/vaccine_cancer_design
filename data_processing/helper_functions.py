...
def generate_peptides(row: Any, peptide_length: int = 9) -> Optional[str]:
    ...
    hugo_symbol = getattr(row, "Hugo_Symbol", row.get("Hugo_Symbol") if hasattr(row, "get") else None)
    wildtype_seq = getattr(row, "wildtype_seq", row.get("wildtype_seq") if hasattr(row, "get") else None)
    ...
    pos = getattr(row, "pos", row.get("pos") if hasattr(row, "get") else None)
    # Handle end_pos if present, else default to pos
    end_pos = getattr(row, "end_pos", row.get("end_pos", pos) if hasattr(row, "get") else pos)

    mut_type = getattr(row, "mut_type", row.get("mut_type") if hasattr(row, "get") else None)
    alt = getattr(row, "alt", row.get("alt") if hasattr(row, "get") else None)
    ...
    try:
        ...
        if mut_type == "Sub":
            ...
            cds_seq = getattr(row, "cds_seq", row.get("cds_seq") if hasattr(row, "get") else None)
