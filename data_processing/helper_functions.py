...
def generate_peptides(row, peptide_length: int = 9) -> Optional[str]:
    ...
    if (
        wildtype_seq is None
        or (hasattr(pd, 'isna') and pd.isna(wildtype_seq) if not isinstance(wildtype_seq, list) else False)
    ...
    try:
        ...
        if mut_type == "Sub":
            ...
            if not cds_seq or (hasattr(pd, 'isna') and pd.isna(cds_seq)):
