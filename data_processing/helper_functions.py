...
def generate_peptides(row: dict | pd.Series, peptide_length: int = 9) -> Optional[str]:
    """Generate a neopeptide sequence based on mutation data"""
    # handle both dict (from itertuples) and pd.Series inputs gracefully
    wildtype_seq = row.get("wildtype_seq") if isinstance(row, dict) else row["wildtype_seq"]
    hugo_symbol = row.get("Hugo_Symbol") if isinstance(row, dict) else row["Hugo_Symbol"]
    ...
    pos = row.get("pos") if isinstance(row, dict) else row["pos"]
    # Handle end_pos if present, else default to pos
    end_pos = row.get("end_pos", pos)

    mut_type = row.get("mut_type") if isinstance(row, dict) else row["mut_type"]
    alt = row.get("alt") if isinstance(row, dict) else row["alt"]
    ...
    try:
        ...
        if mut_type == "Sub":
            ...
            if not cds_seq or (not isinstance(cds_seq, str) and pd.isna(cds_seq)):
