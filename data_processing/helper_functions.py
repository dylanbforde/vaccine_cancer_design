...
def generate_peptides(row: Any, peptide_length: int = 9) -> Optional[str]:
    ...
    # Check if wildtype_seq is a float (like np.nan) because pd.isna throws an error on lists/tuples sometimes
    if not isinstance(wildtype_seq, tuple) or len(wildtype_seq) != 2:
        logging.warning(f"Invalid or missing wildtype_seq for {getattr(row, 'Hugo_Symbol', 'Unknown')}")
        return None

    seq, seq_length = wildtype_seq
    if not seq or seq_length is None:
        logging.warning(f"Missing sequence or length for {getattr(row, 'Hugo_Symbol', 'Unknown')}")
        return None
    ...
    if pos > seq_length or pos <= 0:
        logging.warning(
            f"Isoform mismatch or invalid position: {getattr(row, 'Hugo_Symbol', 'Unknown')} "
    ...
    try:
        ...
        if mut_type == "Sub":
            ...
            if not cds_seq or pd.isna(cds_seq):
                logging.warning(f"Missing CDS for frameshift in {getattr(row, 'Hugo_Symbol', 'Unknown')}")
                return None
    ...
    if len(mutant_peptide) < peptide_length:
        logging.warning(
            f"Peptide too short for {getattr(row, 'Hugo_Symbol', 'Unknown')}: {len(mutant_peptide)} < {peptide_length}. Skipping."
        )
        return None
    ...
    if not all(aa in VALID_AA for aa in mutant_peptide):
        logging.warning(
            f"Invalid peptide generated for {getattr(row, 'Hugo_Symbol', 'Unknown')}: {mutant_peptide}"
        )
        return None
