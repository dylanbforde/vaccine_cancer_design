...
class VaccineDesignPipeline:
    ...
    def process_mutations(self, mutations_df):
        ...
        processed = []
        # ⚡ Bolt: Using itertuples instead of iterrows prevents Series creation overhead per row,
        # resulting in ~18x faster iteration.
        for row in mutations_df.itertuples(index=False):
            peptide = getattr(row, "peptide", None)
            if pd.isna(peptide):
                continue
            ...
            try:
                ...
                graph_data = Data(
                    ...
                    peptide=peptide,
                    mutation_info={
                        "gene": getattr(row, "Hugo_Symbol", None),
                        "sample": getattr(row, "Tumor_Sample_Barcode", None),
                        "position": getattr(row, "pos", None),
                        "mutation": getattr(row, "alt", None),
                    },
                )
